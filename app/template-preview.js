            ...
            <div id="preview-${format.id}" 
                ...
                ${(format.hasText && !state.hideText[format.id]) ? `
                    ...
                    <div id="headline-box-${format.id}" class="absolute" tabindex="0" aria-label="Mover texto ${format.name}" style="width:${TEXT_BOX.widthRatio * 100}%; left:${TEXT_BOX.leftRatio * 100}%;" data-drag-type="text" data-format-id="${format.id}">
                         <div class="rounded-2xl cursor-grab flex items-center relative group transition-all duration-300" 
                            ...
                            <div class="flex-grow min-w-0 flex flex-col items-start text-left relative">
                                <button aria-label="Copiar texto para os outros formatos" data-action="syncHeadline" data-format-id="${format.id}" class="absolute -right-2 -top-2 opacity-0 group-hover:opacity-100 transition-opacity bg-amber-400 text-black p-1 rounded-full shadow-md z-20 hover:bg-amber-500 hover:scale-110 active:scale-95 text-xs font-bold" title="Copiar este texto para os demais formatos">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                                </button>
                                ${state.showEyebrowInput && state.eyebrow ? `<div class="block w-full text-left uppercase mb-2" style="letter-spacing:${glass.eyebrowTrackingEm}em; color: ${templateStyles.eyebrowColor}; font-size:${px(scaleFactor, glass.eyebrowSize)}px; line-height:${px(scaleFactor, glass.eyebrowLineHeight)}px;">${state.eyebrow}</div>` : ''}
                                <div id="headline-text-${format.id}" class="block w-full text-left" data-action="startHeadlineEdit" data-format-id="${format.id}" style="color: ${templateStyles.textColor}; font-size:${px(scaleFactor, glass.headlineSize)}px; line-height:${px(scaleFactor, glass.headlineLineHeight)}px; text-align: left;">
                                ...
                                ${state.showSubtitleInput && state.subtitle ? `<div class="block w-full text-left mt-3" style="color: ${templateStyles.subtitleColor}; font-size:${px(scaleFactor, glass.subtitleSize)}px; line-height:${px(scaleFactor, glass.subtitleLineHeight)}px;">${renderRichTextHtml(state.subtitle)}</div>` : ''}
                            </div>
                        </div>
                    </div>` : ''}
                    ...
                    <div id="headline-box-${format.id}" class="absolute" tabindex="0" aria-label="Mover texto ${format.name}" style="width:${TEXT_BOX.widthRatio * 100}%; left:${TEXT_BOX.leftRatio * 100}%;" data-drag-type="text" data-format-id="${format.id}">
                         <div class="cursor-grab flex flex-col items-start relative group" style="padding: ${px(scaleFactor, gradient.padding)}px;">
                            ...
                            <button aria-label="Copiar texto para os outros formatos" data-action="syncHeadline" data-format-id="${format.id}" class="absolute right-0 top-0 opacity-0 group-hover:opacity-100 transition-opacity bg-amber-400 text-black p-1 rounded-full shadow-md z-20 hover:bg-amber-500 hover:scale-110 active:scale-95 text-xs font-bold" title="Copiar este texto para os demais formatos">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                            </button>
                            ${state.showEyebrowInput && state.eyebrow ? `<div class="block w-full text-left uppercase mb-3 font-bold" style="letter-spacing:${gradient.eyebrowTrackingEm}em; color: ${templateStyles.eyebrowColor}; font-size:${px(scaleFactor, gradient.eyebrowSize)}px; line-height:${px(scaleFactor, gradient.eyebrowLineHeight)}px;">${state.eyebrow}</div>` : ''}
                            <div id="headline-text-${format.id}" class="block w-full text-left" data-action="startHeadlineEdit" data-format-id="${format.id}" style="color: ${templateStyles.textColor}; font-size:${px(scaleFactor, gradient.headlineSize)}px; line-height:${px(scaleFactor, gradient.headlineLineHeight)}px; text-align: left; font-weight: 700; text-shadow: 0px ${px(scaleFactor, gradient.shadowOffsetY)}px ${px(scaleFactor, gradient.shadowBlur)}px rgba(0,0,0,0.5);">
                                ${renderRichTextHtml(state.headlines[format.id])}
                            </div>
                            <textarea id="headline-textarea-${format.id}" data-action="updateHeadline" data-blur-action="finishHeadlineEdit" data-format-id="${format.id}" 
                            ...
                            ${state.showSubtitleInput && state.subtitle ? `<div class="block w-full text-left mt-4" style="color: ${templateStyles.subtitleColor}; font-size:${px(scaleFactor, gradient.subtitleSize)}px; line-height:${px(scaleFactor, gradient.subtitleLineHeight)}px; text-shadow: 0px 2px 8px rgba(0,0,0,0.8);">${renderRichTextHtml(state.subtitle)}</div>` : ''}
                        </div>
                    </div>` : ''}
                    ...
                    <div id="headline-box-${format.id}" class="absolute" tabindex="0" aria-label="Mover texto ${format.name}" style="width:${TEXT_BOX.widthRatio * 100}%; left:${TEXT_BOX.leftRatio * 100}%;" data-drag-type="text" data-format-id="${format.id}">
                         <div class="cursor-grab flex flex-col items-center justify-center relative group text-center" style="padding: ${px(scaleFactor, quote.padding)}px;">
                            ...
                            <div id="headline-text-${format.id}" class="block w-full" data-action="startHeadlineEdit" data-format-id="${format.id}" style="color: ${templateStyles.textColor}; font-size:${px(scaleFactor, quote.headlineSize)}px; line-height:${px(scaleFactor, quote.headlineLineHeight)}px; text-align: center; font-style: italic; font-weight: 700;">
                                ${renderRichTextHtml(state.headlines[format.id])}
                            </div>
                            <textarea id="headline-textarea-${format.id}" data-action="updateHeadline" data-blur-action="finishHeadlineEdit" data-format-id="${format.id}" 
                            ...
                            ${state.showEyebrowInput && state.eyebrow ? `<div class="block w-full font-bold uppercase" style="letter-spacing:${quote.eyebrowTrackingEm}em; color: ${templateStyles.eyebrowColor}; font-size:${px(scaleFactor, quote.eyebrowSize)}px; line-height:${px(scaleFactor, quote.eyebrowLineHeight)}px;">${state.eyebrow}</div>` : ''}
                            ${state.showSubtitleInput && state.subtitle ? `<div class="block w-full" style="margin-top:${px(scaleFactor, quote.subtitleMarginTop)}px; color: ${templateStyles.subtitleColor}; font-size:${px(scaleFactor, quote.subtitleSize)}px; line-height:${px(scaleFactor, quote.subtitleLineHeight)}px;">${renderRichTextHtml(state.subtitle)}</div>` : ''}
                        </div>
                    </div>` : ''}
                    ...
                    <div id="headline-box-${format.id}" class="absolute" tabindex="0" aria-label="Mover texto ${format.name}" style="width:${TEXT_BOX.widthRatio * 100}%; left:${TEXT_BOX.leftRatio * 100}%;" data-drag-type="text" data-format-id="${format.id}">
                         <div class="cursor-grab flex flex-col items-center justify-center relative group text-center" style="padding: ${px(scaleFactor, infographic.padding)}px;">
                            ...
                            ${state.showEyebrowInput && state.eyebrow ? `<div class="block w-full font-bold tracking-tight" style="margin-bottom:${px(scaleFactor, infographic.eyebrowMarginBottom)}px; color: ${templateStyles.eyebrowColor}; font-size:${px(scaleFactor, infographic.eyebrowSize)}px; line-height:${px(scaleFactor, infographic.eyebrowLineHeight)}px; text-shadow: 0px ${px(scaleFactor, infographic.shadowOffsetY)}px ${px(scaleFactor, infographic.shadowBlur)}px rgba(0,0,0,0.5);">${state.eyebrow}</div>` : ''}
                            <div id="headline-text-${format.id}" class="block w-full font-bold uppercase tracking-wider" data-action="startHeadlineEdit" data-format-id="${format.id}" style="color: ${templateStyles.textColor}; font-size:${px(scaleFactor, infographic.headlineSize)}px; line-height:${px(scaleFactor, infographic.headlineLineHeight)}px; text-align: center;">
                                ${renderRichTextHtml(state.headlines[format.id])}
                            </div>
                            <textarea id="headline-textarea-${format.id}" data-action="updateHeadline" data-blur-action="finishHeadlineEdit" data-format-id="${format.id}" 
                            ...
                            ${state.showSubtitleInput && state.subtitle ? `<div class="block w-full" style="margin-top:${px(scaleFactor, infographic.subtitleMarginTop)}px; color: ${templateStyles.subtitleColor}; font-size:${px(scaleFactor, infographic.subtitleSize)}px; line-height:${px(scaleFactor, infographic.subtitleLineHeight)}px; background-color: rgba(0,0,0,0.5); padding: ${px(scaleFactor, infographic.subtitlePaddingY)}px ${px(scaleFactor, infographic.subtitlePaddingX)}px; border-radius: ${px(scaleFactor, infographic.subtitleRadius)}px; display: inline-block;">${renderRichTextHtml(state.subtitle)}</div>` : ''}
                        </div>
                    </div>` : ''}
                ` : ''}
                ...
                ${!isCropping ? `
                 <div class="absolute top-2 right-2 flex gap-2 z-20">
                     <button aria-label="Alterar Preenchimento" data-action="toggleFitMode" data-format-id="${format.id}" class="px-4 py-2 text-xs font-semibold bg-black/60 backdrop-blur-sm rounded-full hover:bg-black/80 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400">
                         ${transform.fitMode === 'contain' ? 'Preencher a tela' : 'Ajustar à tela'}
                     </button>
                     <button aria-label="Exportar ${format.name}" data-action="${state.slides && state.slides.length > 1 ? 'openBatchExportModal' : 'openExportModal'}" data-format-id="${format.id}" class="px-4 py-2 text-xs font-semibold bg-black/60 backdrop-blur-sm rounded-full hover:bg-black/80 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400">
                          ${state.slides && state.slides.length > 1 ? 'Exportar Tudo' : 'Exportar'}
                     </button>
                     <button aria-label="Reenquadrar ${format.name}" data-action="toggleCropMode" data-format-id="${format.id}" class="p-2 bg-black/60 backdrop-blur-sm rounded-full hover:bg-black/80 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400">
                         ${constants.CropIcon}
                     </button>
                 </div>
                 ` : ''}
