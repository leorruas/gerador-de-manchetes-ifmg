...
- `index.js`: ponto de entrada pequeno; a lógica real fica dividida nos arquivos de `app/`.
- `app/core.js`: estado compartilhado, persistência local e helpers centrais.
- `app/handlers-*.js`: eventos de upload, exportação, histórico, crop, texto e drag.
- `app/template-*.js`: templates HTML das telas e previews.
- `app/render.js` e `app/init.js`: renderização central e inicialização.
...
- `services/canvasExport*.js`: exportação final em canvas, helpers e layouts de template.
...
- `PROJECT_PROBLEMS.md`: principais problemas estruturais e riscos de manutenção.
- `BUGS.md`: bugs identificados e resoluções aplicadas.
- `BUSINESS_RULES.md`: regras de negócio e comportamento esperado.
- `STYLE_GUIDE.md`: padrões visuais da interface e das imagens exportadas.
- `EVOLUTION_IDEAS.md`: ideias futuras de produto e arquitetura.
