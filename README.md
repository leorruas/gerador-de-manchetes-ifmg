...
2.  **Import Maps:** Esta é a chave para fazer os `imports` de módulos funcionarem. O `importmap` no `index.html` cria um "mapa" que diz ao navegador como resolver os nomes dos módulos. Para garantir a máxima robustez, a aplicação utiliza o **CDN `esm.sh`** para carregar uma versão estável do React (v18) e mapeia os arquivos locais de forma explícita, usando os nomes de arquivo completos como chaves.
